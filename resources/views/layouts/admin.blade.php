...
<head>
    ...
    <title>@yield('title', 'Quản trị hệ thống') - Đăng Kiểm Tàu Cá</title>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <div class="sidebar-header">
            <a href="#" class="sidebar-brand">
                ...
                <span>
                    CÔNG TY CP ĐĂNG KIỂM TÀU CÁ VÀ DỊCH VỤ HẬU CẦN NGHỀ CÁ NGHỆ AN
                    <small> Quản lý đăng kiểm </small>
                </span>
            </a>
        </div>
        ...
        <div class="sidebar-scroll-area">
            ...
            <div class="sidebar-search px-3 pt-3 pb-2">
                <div class="input-group">
                    <input type="text" class="form-control bg-light border-0 p-3 rounded-4 fw-bold" id="sidebarSearch" placeholder="Tìm kiếm chức năng..." autocomplete="off" style="border-radius: 20px; font-size: 0.85rem;">
                </div>
            </div>
            ...
            <li class="nav-item">
                <a class="nav-link {{ request()->routeIs('admin.proposals.*') ? 'active' : '' }}" href="{{ route('admin.proposals.index') }}">
                    <i class="fas fa-file-contract"></i> <span>Phê duyệt & Đề xuất</span>
                </a>
            </li>
            @endcan
            @if(auth()->user()->can('view_ship') || auth()->user()->can('view_inspection_process') || auth()->user()->can('view_shipyard'))
            <li class="nav-item">
                <a class="nav-link {{ request()->routeIs('admin.ships.*') || request()->routeIs('admin.shipyards.*') || request()->routeIs('admin.inspection-processes.*') ? 'active' : '' }}" 
                   data-bs-toggle="collapse" href="#shipSubmenu" role="button" aria-expanded="{{ request()->routeIs('admin.ships.*') || request()->routeIs('admin.shipyards.*') || request()->routeIs('admin.inspection-processes.*') ? 'true' : 'false' }}">
                    <i class="fas fa-anchor"></i> 
                    <span>Đăng kiểm tàu</span>
                    <i class="fas fa-chevron-down ms-auto arrow" style="font-size: 0.8rem; width: auto;"></i>
                </a>
                <div class="collapse {{ request()->routeIs('admin.ships.*') || request()->routeIs('admin.shipyards.*') || request()->routeIs('admin.inspection-processes.*') ? 'show' : '' }}" id="shipSubmenu" data-bs-parent="#sidebarAccordion">
                    <ul class="nav flex-column collapse-menu">
                        @can('view_ship')
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.ships.index') ? 'active' : '' }}" href="{{ route('admin.ships.index') }}">
                                <i class="fas fa-ship"></i> <span>Danh sách tàu</span>
                            </a>
                        </li>
                        @endcan
                        @can('view_shipyard')
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.shipyards.*') ? 'active' : '' }}" href="{{ route('admin.shipyards.index') }}">
                                <i class="fas fa-industry"></i> <span>Cơ sở đóng mới</span>
                            </a>
                        </li>
                        @endcan
                        @can('view_inspection_process')
                        <li class="nav-item">
                            <a class="nav-link {{ request()->routeIs('admin.inspection-processes.*') ? 'active' : '' }}" href="{{ route('admin.inspection-processes.index') }}">
                                <i class="fas fa-tasks"></i> <span>Quy trình đăng kiểm</span>
                            </a>
                        </li>
                        @endcan
                    </ul>
            ...
            @can('view_crm')
            <li class="nav-item">
                <a class="nav-link {{ request()->routeIs('admin.crm.*') ? 'active' : '' }}" href="{{ route('admin.crm.index') }}">
                    <i class="fas fa-users-cog"></i> <span>Khách hàng (CRM)</span>
                </a>
            </li>
            @endcan
            @can('view_finance')
            ...
            <li class="nav-item">
                <a class="nav-link {{ request()->routeIs('admin.kpi.*') ? 'active' : '' }}" href="{{ route('admin.kpi.index') }}">
                    <i class="fas fa-chart-line"></i> <span>KPI Nhân viên</span>
                </a>
            </li>
            @endcan
            ...
            <li class="nav-item">
                <a class="nav-link {{ request()->routeIs('admin.users.*') || request()->routeIs('admin.roles.*') ? 'active' : '' }}" 
                   data-bs-toggle="collapse" href="#adminSubmenu" role="button" aria-expanded="{{ request()->routeIs('admin.users.*') || request()->routeIs('admin.roles.*') ? 'true' : 'false' }}">
                    <i class="fas fa-user-shield"></i> 
                    <span>Quản trị hệ thống</span>
                    <i class="fas fa-chevron-down ms-auto arrow" style="font-size: 0.8rem; width: auto;"></i>
                </a>
                <div class="collapse {{ request()->routeIs('admin.users.*') || request()->routeIs('admin.roles.*') ? 'show' : '' }}" id="adminSubmenu" data-bs-parent="#sidebarAccordion">
                    <ul class="nav flex-column collapse-menu">
                        @can('view_user')
                        <li class="nav-item">
                    ...
                    <li class="nav-item dropdown no-arrow mx-1">
                        <a class="nav-link dropdown-toggle position-relative" href="#" id="alertsDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            @if($unreadNewsCount > 0)
                                <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size: 0.6rem; margin-top: 10px; margin-left: -10px;">
                                    {{ $unreadNewsCount > 9 ? '9+' : $unreadNewsCount }}
                                    <span class="visually-hidden">unread messages</span>
                                </span>
                            @endif
                        </a>
                        <div class="dropdown-list dropdown-menu dropdown-menu-end shadow animated--grow-in p-0 border-0" aria-labelledby="alertsDropdown" style="width: 300px; max-height: 400px;">
                            <h6 class="dropdown-header text-white p-3 fw-bold rounded-top align-items-center d-flex justify-content-between" style="background: linear-gradient(135deg, #4e73df 0%, #224abe 100%);">
                                <div><i class="fas fa-bullhorn me-1"></i> Tin tức mới</div>
                                @if($unreadNewsCount > 0)
                                    <span class="badge bg-white text-primary rounded-pill">{{ $unreadNewsCount }}</span>
                                @endif
                            </h6>
                            
                            <div class="list-group list-group-flush" style="max-height: 300px; overflow-y: auto;">
                                @forelse($unreadNewsList as $news)
                                    <a class="list-group-item list-group-item-action border-bottom p-3" href="{{ route('admin.news.show', $news->id) }}">
                                        <div class="d-flex align-items-start">
                                            <div class="mr-3 me-3">
                                                <div class="icon-circle bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center" style="width: 36px; height: 36px;">
                                                    <i class="fas fa-file-alt"></i>
                                                </div>
                                            </div>
                                            <div>
                                                <div class="small text-muted mb-1">{{ $news->created_at->diffForHumans() }}</div>
                                                <span class="fw-bold text-dark d-block text-truncate" style="max-width: 200px; font-size: 0.9rem;">{{ $news->title }}</span>
                                            </div>
                                        </div>
                                    </a>
                                @empty
                            ...
                            <a class="dropdown-item text-center small text-primary fw-bold p-3 bg-light border-top" style="border-radius: 0 0 0.5rem 0.5rem;" href="{{ route('admin.news.index') }}">
        ...
        document.getElementById('sidebarSearch')?.addEventListener('input', function(e) {
            ...
            if (searchTerm === '') {
                ...
                submenus.forEach(submenu => {
                    ...
                    if (!submenu.classList.contains('show') && !submenu.parentElement.querySelector('.active')) {
            ...
            sidebarItems.forEach(item => {
                ...
                if (isMatch) {
                    ...
                    if (subMenu) {
                        const hasVisibleChild = Array.from(subMenu.querySelectorAll('.nav-item')).some(child => {
