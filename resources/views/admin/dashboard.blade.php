...
<style>
    ...
    /* Colors and Accents */
    .border-accent-blue { border-bottom: 4px solid #3182ce; }
    .text-blue { color: #2b6cb0; }
    .bg-light-blue { background: rgba(49, 130, 206, 0.1); color: #3182ce; }

    .border-accent-red { border-bottom: 4px solid #e53e3e; }
    .text-red { color: #c53030; }
    .bg-light-red { background: rgba(229, 62, 62, 0.1); color: #e53e3e; }

    .border-accent-orange { border-bottom: 4px solid #ed8936; }
    .text-orange { color: #c05621; }
    .bg-light-orange { background: rgba(237, 137, 54, 0.1); color: #ed8936; }

    .border-accent-purple { border-bottom: 4px solid #805ad5; }
    .text-purple { color: #6b46c1; }
    .bg-light-purple { background: rgba(128, 90, 213, 0.1); color: #805ad5; }

    .border-accent-green { border-bottom: 4px solid #48bb78; }
    .text-green { color: #2f855a; }
    .bg-light-green { background: rgba(72, 187, 120, 0.1); color: #48bb78; }
</style>
...
<div class="mb-4" style="background:#fff5f5;border:1px solid #fed7d7;border-left:4px solid #e53e3e;border-radius:12px;padding:14px 20px;display:flex;align-items:center;justify-content:space-between;gap:16px;animation:slideInBanner 0.4s ease;">
    <div style="display:flex;align-items:center;gap:14px;">
        <div style="width:40px;height:40px;background:#fed7d7;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <i class="fas fa-bell" style="color:#e53e3e;font-size:1.1rem;"></i>
        </div>
        <div>
            <div style="font-weight:700;color:#c53030;font-size:0.95rem;">Nhắc nhở bảo trì hôm nay!</div>
            <div style="color:#718096;font-size:0.85rem;">
                Có <strong style="color:#e53e3e;">{{ $overdueElevators->count() }}</strong> thang máy đã đến hạn hoặc quá hạn bảo trì theo chu kỳ.
            </div>
        </div>
    </div>
    <a href="{{ route('admin.maintenance.index') }}" style="background:#e53e3e;color:#fff;border-radius:8px;padding:7px 18px;font-size:0.85rem;font-weight:700;text-decoration:none;white-space:nowrap;transition:opacity 0.2s;" onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">
        Xem tất cả
    </a>
</div>
@endif
...
<div class="mb-4" style="background:#ebf8ff;border:1px solid #bee3f8;border-left:4px solid #3182ce;border-radius:12px;padding:14px 20px;display:flex;align-items:center;justify-content:space-between;gap:16px;animation:slideInBanner 0.4s ease;">
    <div style="display:flex;align-items:center;gap:14px;">
        <div style="width:40px;height:40px;background:#bee3f8;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        ...
        <div>
            ...
            <div style="color:#718096;font-size:0.85rem;">
                Bạn có <strong style="color:#3182ce;">{{ $staffUpcomingMaintenance->count() }}</strong> lịch bảo trì trong 7 ngày tới:
                @foreach($staffUpcomingMaintenance->take(3) as $m)
                    <span style="display:inline-block;background:#dbeafe;color:#1d4ed8;font-size:0.75rem;font-weight:600;padding:2px 8px;border-radius:6px;margin-left:4px;">
                        {{ $m->elevator->code ?? 'N/A' }} — {{ \Carbon\Carbon::parse($m->check_date)->format('d/m') }}
                    </span>
                @endforeach
                @if($staffUpcomingMaintenance->count() > 3)
                    <span style="color:#a0aec0;font-size:0.8rem;"> +{{ $staffUpcomingMaintenance->count() - 3 }} khác</span>
                @endif
            </div>
        </div>
    </div>
    <a href="{{ route('admin.maintenance.index') }}" style="background:#3182ce;color:#fff;border-radius:8px;padding:7px 18px;font-size:0.85rem;font-weight:700;text-decoration:none;white-space:nowrap;transition:opacity 0.2s;" onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">
        Xem lịch
    </a>
</div>
@endif
...
<style>
    ...
    .reminder-list, .customer-list, .order-list {
        max-height: 390px;
        overflow-y: auto;
        padding-right: 5px;
    }
    /* Tùy chỉnh thanh cuộn cho danh sách */
    .reminder-list::-webkit-scrollbar, .customer-list::-webkit-scrollbar, .order-list::-webkit-scrollbar {
        width: 4px;
    }
    .reminder-list::-webkit-scrollbar-track, .customer-list::-webkit-scrollbar-track, .order-list::-webkit-scrollbar-track {
        background: #f1f1f1;
        border-radius: 10px;
    }
    .reminder-list::-webkit-scrollbar-thumb, .customer-list::-webkit-scrollbar-thumb, .order-list::-webkit-scrollbar-thumb {
        background: #e2e8f0;
        border-radius: 10px;
    }
    .reminder-list::-webkit-scrollbar-thumb:hover, .customer-list::-webkit-scrollbar-thumb:hover, .order-list::-webkit-scrollbar-thumb:hover {
        background: #cbd5e0;
    }
    .order-item {
        display: flex;
        ...
        <div class="chart-card">
            ...
            <div class="reminder-list mt-3">
                ...
                <div class="reminder-card">
                    ...
                    <div class="text-end">
                        <div class="small fw-bold" style="color: {{ $reminder['color'] }}">{{ $reminder['label'] }}</div>
                        <div class="small fw-bold" style="color: {{ $reminder['color'] }}">{{ $reminder['deadline'] }}</div>
                        <div class="text-muted" style="font-size: 0.7rem;">Chu kỳ: {{ $reminder['cycle'] }} ngày</div>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="chart-card">
            ...
            <div class="customer-list mt-2">
                ...
                <div class="customer-item">
                    <div class="priority-circle" style="background: {{ $customer['days_left'] <= 0 ? '#fff5f5' : ($customer['days_left'] <= 5 ? '#fffaf0' : '#ebf8ff') }}; color: {{ $customer['days_left'] <= 0 ? '#c53030' : ($customer['days_left'] <= 5 ? '#c05621' : '#2b6cb0') }};">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-1">
                            <div class="text-muted" style="font-size: 0.75rem;">{{ $customer['customer_name'] }}</div>
                            <span class="badge-priority {{ $customer['badge_class'] }}">{{ $customer['priority'] }}</span>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <div class="chart-card">
            ...
            <div class="order-list mt-2">
                ...
                <div class="order-item">
                    <div class="bg-light-blue p-2 rounded" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center;">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-1">
                            <div class="text-muted" style="font-size: 0.75rem;">{{ $order['building_name'] }}</div>
                            <span class="badge-status {{ $order['status_class'] }}">{{ $order['status_label'] }}</span>
                        </div>
                    </div>
                </div>
                @endforeach
