    ...
    <div class="mb-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                ...
                <p class="mb-0 text-muted" style="font-size: 0.95rem;">Dữ liệu tổng hợp và báo cáo chi tiết tháng
                    {{ str_pad($month, 2, '0', STR_PAD_LEFT) }} năm {{ $year }}.</p>
            </div>

            <div class="d-flex align-items-center gap-3">
                <div class="month-picker-container">
                    <a href="?month={{ $prevMonth->month }}&year={{ $prevMonth->year }}"
                        class="month-picker-btn text-decoration-none">
        ...
        <div class="tab-pane fade show active" id="overview" role="tabpanel">
            <div class="row g-4">
                ...
                <div class="col-lg-7">
                    <div class="report-card p-4 h-100">
                        <h5 class="fw-bold text-dark mb-4 d-flex align-items-center">
                            <i class="fas fa-user-friends text-primary me-2"></i> Hiệu suất Kỹ thuật viên (Số task hoàn
                            thành)
                        </h5>
                        <div style="position: relative; height: 300px; width: 100%;">
            ...
            <div class="report-card p-0 overflow-hidden">
                <div class="p-4 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <h5 class="fw-bold text-dark mb-0 d-flex align-items-center">
                        <i class="far fa-calendar-alt text-primary me-2"></i> Danh sách thang máy cần bảo trì
                        {{ $strMonthYear }}
                    </h5>
                    <a href="{{ route('admin.reports.export_maintenance', ['month' => $month, 'year' => $year]) }}"
                        class="btn btn-sm btn-outline-success fw-bold px-3 shadow-sm">
                ...
                <div class="table-responsive">
                    <table class="table table-report">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            ...
                                            $checkDate = $elv->next_check_date
                                                ? $elv->next_check_date->startOfDay()
                                                : null;
                                            $isOverdue = $checkDate && $checkDate < $today;
                                            $isDue =
                                                $checkDate &&
                                                !$isOverdue &&
                                                $checkDate->diffInDays($today, false) >= -3;
                                            $isSoon = $checkDate && !$isOverdue && !$isDue;
                                        @endphp
                                        @if ($isOverdue)
                                            <span class="badge-status status-qua-han"><i
                                                    class="fas fa-exclamation-triangle me-1"
                                                    style="font-size:0.75rem;"></i>Quá hạn</span>
                                        @elseif($isDue)
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center text-muted py-4">Không có thang máy nào cần bảo
                                        trì trong tháng này.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="tab-pane fade" id="staff" role="tabpanel">
            <div class="report-card p-0 overflow-hidden">
                <div class="p-4 border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <a href="{{ route('admin.reports.export_staff', ['month' => $month, 'year' => $year]) }}"
                        class="btn btn-sm btn-outline-success fw-bold px-3 shadow-sm">
                        <i class="fas fa-file-excel me-1"></i> Xuất Excel
                    </a>
                </div>
                            ...
                            @forelse($staffPerformance as $staff)
                                <tr>
                                    ...
                                    <td class="text-center"><span
                                            class="count-badge count-completed">{{ $staff['completed'] }}</span></td>
                                    <td class="text-center"><span
                                            class="count-badge count-progress">{{ $staff['in_progress'] }}</span></td>
                                    <td class="text-center"><span
                                            class="count-badge count-pending">{{ $staff['pending'] }}</span></td>
                                    <td class="text-center fw-bold fs-5 text-dark">{{ $staff['total'] }}</td>
                                    <td class="text-center">
                                        <span class="rating-star"><i class="fas fa-star me-1"></i>
                                            {{ number_format($staff['rating'], 1) }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center text-muted py-4">Không có dữ liệu trong tháng
                                        này.</td>
                                </tr>
                            @endforelse
                        </tbody>
