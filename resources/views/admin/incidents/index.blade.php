...
<style>
    ...
    .priority-emergency { background: #ef4444; color: #fff; box-shadow: 0 4px 10px rgba(239, 68, 68, 0.2); }
    ...
    .status-processing { background: #fefce8; color: #a16207; border: 1px solid #fef08a; }
    .status-resolved { background: #f0fdf4; color: #15803d; border: 1px solid #dcfce7; }
    .status-canceled { background: #f9fafb; color: #6b7280; border: 1px solid #e5e7eb; }
    ...
    .btn-action-view:hover { color: #3b82f6; border-color: #3b82f6; }
    .btn-action-edit:hover { color: #10b981; border-color: #10b981; }
    ...
    .stat-card-emergency { background: #fff5f5; border-color: #fee2e2; }
    .stat-card-emergency .stat-card-label { color: #e11d48; }
    .stat-card-emergency .stat-card-value { color: #e11d48; }

    .stat-card-processing { background: #f0f7ff; border-color: #dbeafe; }
    .stat-card-processing .stat-card-label { color: #2563eb; }
    .stat-card-processing .stat-card-value { color: #2563eb; }
</style>
@endsection
    ...
    <form action="{{ route('admin.incidents.index') }}" method="GET">
        <div class="row g-3">
            <div class="col-lg-3 col-md-6">
                ...
                <input type="text" name="search" class="form-control filter-input" placeholder="VD: INC-2026-001..." value="{{ request('search') }}">
            </div>
            <div class="col-lg-3 col-md-6">
                <label class="filter-label small"><i class="far fa-building"></i> Tòa nhà / Khách hàng</label>
                <input type="text" name="building" class="form-control filter-input" placeholder="Tìm kiếm khách hàng..." value="{{ request('building') }}">
            </div>
            <div class="col-lg-2 col-md-6">
                ...
                <select name="priority" class="form-select filter-input">
                    ...
                    <option value="emergency" {{ request('priority') == 'emergency' ? 'selected' : '' }}>Khẩn cấp</option>
...
<div class="tech-card overflow-hidden">
    <div class="table-responsive">
        <table class="table table-incident mb-0">
            <thead>
                <tr>
                    <th style="width: 150px;">Mã sự cố</th>
                    <th style="width: 250px;">Thang máy & Khách hàng</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="fw-bold text-dark fs-5">{{ $incident->code }}</td>
                    <td>
                        <div class="fw-bold text-primary mb-1">{{ $incident->elevator->code ?? 'N/A' }}</div>
                        <div class="small text-muted">{{ $incident->elevator->building->name ?? 'N/A' }}</div>
                    </td>
                    <td>
                        <div class="fw-semibold text-dark mb-2">{{ $incident->description }}</div>
                        <div class="d-flex align-items-center gap-3 reporter-info">
                            <span><i class="far fa-user me-1"></i> {{ $incident->reporter_name }}</span>
                            <span><i class="fas fa-phone-alt me-1"></i> {{ $incident->reporter_phone }}</span>
                        </div>
                    </td>
                    <td class="text-center">
                        @php
                            $pLabel = ['emergency' => 'Khẩn cấp', 'high' => 'Cao', 'medium' => 'Trung bình', 'low' => 'Thấp'][$incident->priority] ?? $incident->priority;
                    ...
                    <td class="text-center">
                        @php
                            $sLabel = ['new' => 'MỚI BÁO', 'processing' => 'ĐANG XỬ LÝ', 'resolved' => 'HOÀN THÀNH', 'canceled' => 'ĐÃ HỦY'][$incident->status] ?? $incident->status;
                            $sClass = 'status-' . $incident->status;
                        @endphp
                        <span class="badge-status-dot {{ $sClass }}">{{ $sLabel }}</span>
                    </td>
                    <td>
                        <div class="fw-semibold text-dark">{{ $incident->reported_at ? $incident->reported_at->format('d/m/Y') : '' }}</div>
                        <div class="small text-muted">{{ $incident->reported_at ? $incident->reported_at->format('H:i') : '' }}</div>
                    </td>
                    <td class="text-center">
                        <div class="d-flex justify-content-center gap-2">
                            <a href="{{ route('admin.incidents.show', $incident->id) }}" class="btn-action btn-action-view" data-bs-toggle="tooltip" title="Xem chi tiết">
                                <i class="far fa-eye"></i>
                            </a>
                            <a href="{{ route('admin.incidents.edit', $incident->id) }}" class="btn-action btn-action-edit" data-bs-toggle="tooltip" title="Sửa">
                                <i class="far fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.incidents.destroy', $incident->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Xác nhận xóa sự cố này?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-action btn-action-delete" data-bs-toggle="tooltip" title="Xóa">
