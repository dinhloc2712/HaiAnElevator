...
@section('title', 'Chỉnh sửa đơn lắp đặt')
...
@section('content')
    <div class="tech-header-container mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h1 class="h3 mb-1 text-gray-800 fw-bold">Chỉnh sửa đơn lắp đặt</h1>
                <p class="mb-0 text-muted small">Cập nhật thông tin tiến độ và nhân sự phụ trách</p>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-header bg-primary text-white p-4 border-0">
                    <h5 class="mb-0 fw-bold"><i class="fas fa-edit me-2"></i> Thông tin đơn hàng: {{ $installation->code }}</h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('admin.installations.update', $installation->id) }}" method="POST">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="code" class="form-control bg-light border-0 p-3 rounded-end-4 fw-bold" 
                                           placeholder="Ví dụ: INST-001" value="{{ old('code', $installation->code) }}" required>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="branch_id" class="form-select modern-form-control p-3 bg-light border-0 fw-bold" required>
                                    ...
                                    @foreach($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch_id', $installation->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12">
                                ...
                                <select name="building_id" id="building_select" class="form-select select2-tag-enable" required style="width: 100%;">
                                    ...
                                    @foreach($buildings as $building)
                                        <option value="{{ $building->id }}" data-phone="{{ $building->contact_phone }}" {{ old('building_id', $installation->building_id) == $building->id ? 'selected' : '' }}>{{ $building->name }} - {{ $building->address }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="phone" class="form-control bg-light border-0 p-3 rounded-end-4 fw-bold" 
                                           placeholder="Ví dụ: 0912..." value="{{ old('phone', $installation->phone) }}">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="user_id" class="form-select modern-form-control p-3 bg-light border-0 fw-bold" required>
                                    ...
                                    @foreach($staffs as $staff)
                                        <option value="{{ $staff->id }}" {{ old('user_id', $installation->user_id) == $staff->id ? 'selected' : '' }}>{{ $staff->name }} ({{ $staff->role->display_name ?? 'NV' }})</option>
                                    @endforeach
                                </select>
                            ...
                            {{-- Trạng thái --}}
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted text-uppercase">Trạng thái hiện tại</label>
                                <select name="status" class="form-select modern-form-control p-3 bg-light border-0 fw-bold" required>
                                    <option value="pending" {{ old('status', $installation->status) == 'pending' ? 'selected' : '' }}>Chờ giao (Pending)</option>
                                    <option value="in_progress" {{ old('status', $installation->status) == 'in_progress' ? 'selected' : '' }}>Đang lắp (In Progress)</option>
                                    <option value="completed" {{ old('status', $installation->status) == 'completed' ? 'selected' : '' }}>Đã xong (Completed)</option>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="start_date" class="form-control bg-light border-0 p-3 rounded-4 fw-bold" value="{{ old('start_date', $installation->start_date ? $installation->start_date->format('Y-m-d') : '') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-muted text-uppercase">Ngày dự kiến hoàn thành</label>
                                <input type="date" name="due_date" class="form-control bg-light border-0 p-3 rounded-4 fw-bold" value="{{ old('due_date', $installation->due_date ? $installation->due_date->format('Y-m-d') : '') }}">
                            </div>

                            {{-- Ghi chú --}}
                            <div class="col-12">
                                <label class="form-label small fw-bold text-muted text-uppercase">Ghi chú thêm</label>
                                <textarea name="notes" class="form-control bg-light border-0 p-3 rounded-4" rows="3" placeholder="Yêu cầu đặc biệt hoặc thông tin lưu ý...">{{ old('notes', $installation->notes) }}</textarea>
                            </div>
                        </div>
