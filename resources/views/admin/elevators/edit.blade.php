...
@section('title', 'Chỉnh sửa thang máy')
...
@section('content')
    <div class="row mb-4 align-items-center">
        <div class="col-md-6">
            <h1 class="h3 mb-0 text-gray-800 fw-bold">Chỉnh sửa: {{ $elevator->code }}</h1>
        </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row mt-3">
                            <div class="col-12 mb-3">
                                ...
                                <input type="text" name="address" class="form-control modern-form-control @error('address') is-invalid @enderror"
                                    value="{{ old('address', $elevator->address) }}" placeholder="Số nhà, tên đường, thôn/xóm...">
                                @error('address')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mt-3">
                            <div class="col-12 mb-3">
                                ...
                                <div id="map"></div>
                                <input type="hidden" name="map" id="map-coords" value="{{ old('map', $elevator->map) }}">
                        ...
                        <div class="row">
                            <div class="col-4 mb-3">
                                ...
                                <input type="text" name="manufacturer" class="form-control modern-form-control"
                                    value="{{ old('manufacturer', $elevator->manufacturer) }}">
                            </div>
                            <div class="col-4 mb-3">
                                ...
                                <input type="text" name="model" class="form-control modern-form-control"
                                    value="{{ old('model', $elevator->model) }}">
                            </div>
                            <div class="col-4 mb-3">
                                ...
                                <input type="number" name="floors" class="form-control modern-form-control"
                                    value="{{ old('floors', $elevator->floors) }}">
                            </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="capacity" class="form-control modern-form-control"
                                    value="{{ old('capacity', $elevator->capacity) }}">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold small text-uppercase text-muted">Ghi chú</label>
                            <textarea name="note" class="form-control modern-form-control" rows="3">{{ old('note', $elevator->note) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="maintenance_end_date" class="form-control modern-form-control"
                                value="{{ old('maintenance_end_date', $elevator->maintenance_end_date ? $elevator->maintenance_end_date->format('Y-m-d') : '') }}">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="inspection_date" class="form-control modern-form-control @error('inspection_date') is-invalid @enderror"
                                value="{{ old('inspection_date', $elevator->inspection_date ? $elevator->inspection_date->format('Y-m-d') : '') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status_disabled" class="form-select modern-form-control bg-light" disabled>
                                <option value="active" {{ old('status', $elevator->status) == 'active' ? 'selected' : '' }}>Hoạt động</option>
                                <option value="error" {{ old('status', $elevator->status) == 'error' ? 'selected' : '' }}>Lỗi</option>
                                <option value="maintenance" {{ old('status', $elevator->status) == 'maintenance' ? 'selected' : '' }}>Bảo trì</option>
                            </select>
                            <input type="hidden" name="status" value="{{ $elevator->status }}">
                            <small class="text-muted mt-1 d-block"><i class="fas fa-info-circle me-1"></i> Trạng thái thiết bị được cập nhật tự động qua hệ thống.</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2">
                    <button type="submit" class="btn btn-tech-primary btn-lg fw-bold">
                        <i class="fas fa-save me-2"></i> Cập nhật thang máy
                    </button>
                    <a href="{{ route('admin.elevators.index') }}" class="btn btn-outline-secondary fw-bold">Hủy bỏ</a>
                </div>
            </div>
        </div>
    </form>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const oldCoords = coordsInput.value;
